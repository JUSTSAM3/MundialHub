...
/**
 * Clase de configuración para OpenAPI (Swagger). Proporciona la configuración
 * básica para la documentación automática de la API REST.
 * 
 * @since 1.0
 */
@Configuration
public class OpenApiConfig {
	/**
	 * Configura los datos básicos del OpenAPI para la documentación. Incluye
	 * descripción de la API, esquema de seguridad JWT, y ejemplos de respuestas
	 * comunes.
	 * 
	 * @return Instancia de OpenAPI configurada con la información de la API.
	 */
	@Bean
	public OpenAPI customOpenAPI() {
		...
		String mainDescription = "<h2>Guía para principiantes de la API REST</h2>"
				...
				+ "    <li><strong>JWT (JSON Web Token)</strong>: Un estándar para crear tokens de acceso. Cuando inicias sesión, "
				+ "        recibirás un token que debes incluir en las cabeceras de tus peticiones posteriores.</li>"
				+ "    <li><strong>Autenticación</strong>: Proceso de verificar tu identidad mediante credenciales (usuario y contraseña).</li>"
				+ "    <li><strong>Autorización</strong>: Determina qué acciones puede realizar un usuario autenticado según su rol.</li>"
				+ "</ul>" + "<h3>Flujo básico de uso:</h3>" + "<ol>"
				+ "    <li>Registra un nuevo usuario usando <code>/auth/register</code></li>"
				+ "    <li>Inicia sesión con <code>/auth/login</code> para obtener un token JWT</li>"
				+ "    <li>Incluye el token en el encabezado de autorización de tus peticiones: <code>Authorization: Bearer tu_token_jwt</code></li>"
				+ "    <li>Usa los endpoints de usuario para gestionar usuarios (requiere autenticación)</li>" + "</ol>"
				+ "<h3>Roles de usuario:</h3>" + "<ul>"
				+ "    <li><strong>USER</strong>: Puede ver información de usuarios</li>"
				+ "    <li><strong>ADMIN</strong>: Puede crear, actualizar y eliminar usuarios además de ver información</li>"
		...
		Info info = new Info().title("API de Primera Aplicación Spring").version("1.0").description(mainDescription)
		...
		return new OpenAPI().info(info).components(new Components().addSecuritySchemes("bearerAuth", securityScheme)
				...
				.addResponses("NotFoundError",
						new ApiResponse().description("Recurso no encontrado").content(new Content().addMediaType(
								"application/json", new MediaType().addExamples("error", new Example().value(
